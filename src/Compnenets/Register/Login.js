...
import {Goto, saveUser}from '../General/Helper';
...
function Login (){
    ...
    const handleSubmit=async(e)=>{
        e.preventDefault();
        try{
            const data=isValid?await saveUser(API_URL.LOGIN,{...currentUser}):null;
            if(data.status===201)
                Goto(`/user/profile/${data.data.id}`);
    ...
    return(
        <div className="w-full h-full main flex flex-col items-center">

        <div className="flex justify-center  center_parent w-3/5  ">
